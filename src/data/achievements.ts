...
export const ACHIEVEMENT_GROUPS: AchievementGroupDef[] = [
  ...
  { id: 'outcome', name: '关键成果', desc: '做出被认可的核心产出', ids: ['first-gold', 'first-silver', 'first-copper', 'silver-3', 'copper-3', 'gold-3', 'awards-10', 'grand-slam'] },
...
export const ACHIEVEMENTS: AchievementDef[] = [
  ...
  },
  {
    ...
    name: '银牌新秀',
    desc: '首次获得银牌关键成果。',
    icon: medalIcon('first-silver'),
    check: (_e, m) => Object.values(m).some((x) => x.award === 'silver'),
    progress: (_e, m) => ({ current: Math.min(Object.values(m).filter((x) => x.award === 'silver').length, 1), target: 1 }),
  },
  {
    id: 'first-copper',
    name: '铜牌新星',
    desc: '首次获得铜牌关键成果。',
    icon: medalIcon('first-copper'),
    check: (_e, m) => Object.values(m).some((x) => x.award === 'copper'),
    progress: (_e, m) => ({ current: Math.min(Object.values(m).filter((x) => x.award === 'copper').length, 1), target: 1 }),
  },
  {
    id: 'silver-3',
    name: '银牌进阶',
  ...
  },
  {
    ...
    name: '铜牌高手',
    desc: '累计获得 3 个铜牌关键成果。',
    icon: medalIcon('copper-3'),
    check: (_e, m) => Object.values(m).filter((x) => x.award === 'copper').length >= 3,
    progress: (_e, m) => ({ current: Math.min(Object.values(m).filter((x) => x.award === 'copper').length, 3), target: 3 }),
  },
  {
    id: 'gold-3',
    name: '金牌大师',
