...
基于 iCode（代码评审/提交）与 iCafe（需求卡片）真实数据生成的任务看板，用于记录个人产出、按工作大类归类、沉淀每件事的收获与反思。适合实习转正汇报、年终总结的场景。
...
- **任务看板**：把归并后的每件事（卡片/CR/提交）自动分类到五大工作大类——功能实现 / 用户体验 / 工作提效 / Bug修复 / 工程建设
- **拖拽归类**：拖动卡片到任一分类即可手动归类（dnd-kit），分类结果自动持久化到 localStorage
- **难度打分 + 反思**：点击卡片可对每件事打难度分（1-5 星）并写总结反思
- **导出总结**：一键导出为 Markdown，按分类组织，直接用于汇报
